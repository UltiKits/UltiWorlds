...
- **Target**, exactly four: `player`, `console`, `both`, or `n/a` — the first three read straight
  ...
  Kind. `WorldCommand` carries class-level `@CmdTarget(BOTH)` and a method-level
  `@CmdTarget(PLAYER)` on 20 of its 21 mappings. A row's Target describes who can reach the
...
| ultiworlds.world.delete | Permanently delete a world: unload it if loaded, then remove its on-disk folder and its `WorldSettings` row. Refuses the configured default world and any world listed in `protected_worlds` (`world`/`world_nether`/`world_the_end` by shipped default), compared without regard to letter case; a refused world keeps both its folder and its settings row. The protected refusal is applied in `WorldService#deleteWorld`, not in the command, so it covers every caller — the command only picks which message the sender sees (`world.delete.default` for the default world, `world.delete.protected` for a listed one). The command itself deletes nothing: after those refusals it opens the `ultiworlds.gui.world-delete-confirm` window for the named world, and the deletion happens only when the player presses that window's OK button; Cancel, or closing the window any other way, deletes nothing (`UltiKits/UltiWorlds#19`). The server console can run it too, and confirms by typing: the first `/world delete <name>` from the console makes the same refusals, then deletes nothing and answers `world.delete.confirm_console` (the name, and that the same command must be run again within 30 seconds); repeating the same command, for the same name exactly as typed, within 30 seconds of that request runs every refusal again and then deletes, reporting `world.delete.deleting` then `world.delete.success` or `world.delete.failed`. A repeat made after this module has deleted a world by that name for another request (from the page, the console or anywhere else through `WorldService#deleteWorld`) deletes nothing: it answers `world.delete.invalidated`, then the confirmation line again, and counts as a new request. Known limit: neither confirmation recognises "the same world" — a world deleted by another plugin or by hand and created again under the same name within the 30 seconds is deleted by the earlier confirmation (maintainer decision of 2026-09-24). A repeat after 30 seconds deletes nothing and starts a new 30-second window; a different name, or the same name in other letter case, neither confirms nor cancels the pending one; a request confirms at most one deletion; and a refusal on the request or on the repeat discards the request. The window is held in memory for the console sender and does not survive a restart, and it is measured on the JVM's monotonic clock, so changing the system time neither lengthens nor shortens it. Any other non-player sender (a command block, a command minecart, an `/execute` proxy, and RCON — `RemoteConsoleCommandSender` is not a `ConsoleCommandSender`; maintainer decision of 2026-09-24 that RCON stays refused) is refused with `world.delete.sender_not_allowed` and deletes nothing (`UltiKits/UltiWorlds#19`, maintainer decision of 2026-09-24). Every sender that is a `ConsoleCommandSender` counts as the console (maintainer decision of 2026-09-24), and every one of them reports the name `CONSOLE`, so all of them share one pending confirmation per world name and any one confirms a request made by any other: the server console, UltiPanel remote commands (the framework's `CommandExecutionManager` dispatches through a `ConsoleCommandSender` that reports the console's name), post-teleport commands, and console commands run by other modules or plugins (for example UltiEssentials scheduled commands, UltiMenu actions, UltiKits kit commands, UltiMail `console:` commands). A panel user never sees the prompt, the outcome or a refusal: `CommandExecutionManager` reads the command's output as soon as `Bukkit#dispatchCommand` returns, but this command's body runs one tick later, so the panel's result always reads "Command executed successfully" and the prompt, outcome and refusals appear only as server log lines. Post-teleport commands (`ultiworlds.world.postcmd-add`, which needs `ultiworlds.admin.settings`) run as `Bukkit#getConsoleSender` from `WorldService#executePostTeleportCommands`, called only by `WorldService#teleportToWorld` (`/world tp`, or a click in the world list); a `world delete <name>` stored there is confirmed by any two such teleports into that world within 30 seconds, by the same or different players, or by one such teleport plus the same command from any console-identity sender, and the prompt goes to the console, never to the teleporting players. Never deletes through a symbolic link, wherever the link is found: a link among the world folder's children has only its entry removed, and so does a world folder that is itself a link — otherwise `File#listFiles()` resolves the link and the target's contents are deleted through it while only the link is reported as the world (`UltiKits/UltiWorlds#20`). A link is therefore not a route to a protected world's data under an unprotected name, and the console states when the named thing is a link, phrased as what the command can reach rather than as what it has already done, so the line remains true when the link cannot be removed; a failed unlink is then reported as a link still in the world container rather than as files remaining in a folder | command | `/world delete <name>` | ultiworlds.admin.delete | both | admin | detailed | WorldCommand#deleteWorld, WorldDeleteConfirmPage#onConfirm, ConsoleDeleteConfirmations#confirm, WorldService#deleteWorld |
...
| ultiworlds.world.help | Print `/world` usage — a base block (`help.*` keys) plus, only for a sender holding `ultiworlds.admin`, an admin block; the admin block's `difficulty`/`postcmd` lines are read from a SEPARATE `command.help.*` namespace than the rest of the block, a fact this module's own `lang/en.yml` comment (line 207) already documents, not a silent inconsistency. The admin block's `help.delete` line says the command asks for confirmation first. From the server console, `/world help` prints only `help.header` and `help.delete_console` (the one subcommand the console can run, and its 30-second repeat); from any other non-player sender it prints `world.delete.sender_not_allowed` — the framework answers the literal `help` argument before matching a mapping, gated by the class-level target, which admits non-players since `UltiKits/UltiWorlds#19` | command | bare `/world help`, or literal `/world help` argument (the `@CmdMapping(format = "help")` site itself is unreachable through `matchMethod`, per this section's own note above) | ultiworlds.use | both | player | none | WorldCommand#handleHelp |
...
| ultiworlds.world.load | Bring an on-disk-but-unloaded world back online, rebuilt with the environment its own folder shows rather than as an overworld: the service reads the dimension sub-folder the server wrote inside the world folder, and reads it again on every call, so the environment survives a restart and describes the folder as it is at the moment the command runs rather than as it was earlier in the session (`UltiKits/UltiWorlds#22`, fixed). Nothing is carried between calls: a world folder is mutable between two commands, so a remembered answer would be true only of the moment it was taken, and an operator who repairs a folder would never have the repaired folder read. The folder reading is deliberately unwilling, and it is announced whenever it makes or refuses a decision: a top-level `DIM-1` alone means NETHER and a top-level `DIM1` alone means THE_END, each announced at WARNING naming the world and the marker it read; a symlinked dimension entry, both dimension directories at once (a single-player save layout), or a dimension directory sitting beside a top-level `region` directory (the shape `#22` produced) are all ambiguous, so nothing is inferred, the server's own default applies exactly as it did before this version, and a WARNING states what was observed. Every one of those lines reports an observation and points at the changelog entry and `UltiKits/UltiWorlds#22` for the procedure; none of them prescribes one, because a remedy for a folder the module has just declared it cannot read would be the same guess the refusal exists to avoid. A world with no dimension entry at all is the ordinary case: nothing inferred, nothing logged; an entry that is a symbolic link leading nowhere is NOT that case and is reported like the other refusals. Accepts a name that is either currently loaded (no-op success) or present as an on-disk folder, unlike `## GUI`'s `ultiworlds.gui.world-list-page` which only ever lists already-loaded worlds | command | `/world load <name>` | ultiworlds.admin.load | player | admin | brief | WorldCommand#loadWorld, WorldService#loadWorld |
...
| ultiworlds.config.worlds.messages.world_deleted | Declared as the world-deleted message; never read — the actual `/world delete` success line comes from `command.delete.success` via `plugin.i18n(...)`, sent by the confirmation window's OK button (`UltiKits/UltiWorlds#19`) | config | `config/worlds.yml: messages.world_deleted (default: Simplified Chinese text, not reproduced per D-02, has no effect)` | n/a | n/a | admin | none | WorldConfig#worldDeletedMessage (declared, never read outside this class) |
...
| ultiworlds.config.worlds.protected_worlds | This key's own declared comment (`WorldConfig.java:31`) reads "Worlds that cannot be auto-unloaded or deleted"; both halves are enforced — the empty-world scheduled task skips a listed world, and `WorldService#deleteWorld` refuses to delete one, removing neither its folder nor its settings row (`UltiKits/UltiWorlds#20`, fixed). Both call sites compare names without regard to letter case, through one shared helper, so the two halves cannot drift apart again. The list does NOT restrict `/world unload`, which the key's comment does not claim: only the *auto*-unload task consults it | config | `config/worlds.yml: protected_worlds (default: [world, world_nether, world_the_end])` | n/a | n/a | admin | detailed | WorldService#checkAutoUnloadEmptyWorlds, WorldService#deleteWorld, WorldService#isDeleteProtected |
