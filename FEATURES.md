...
| ultiworlds.world.delete | Permanently delete a world: unload it if loaded, then remove its on-disk folder and its `WorldSettings` row. Refuses the configured default world and any world listed in `protected_worlds` (`world`/`world_nether`/`world_the_end` by shipped default), compared without regard to letter case; a refused world keeps both its folder and its settings row. The protected refusal is applied in `WorldService#deleteWorld`, not in the command, so it covers every caller — the command only picks which message the sender sees (`world.delete.default` for the default world, `world.delete.protected` for a listed one). The command itself deletes nothing: after those refusals it opens the `ultiworlds.gui.world-delete-confirm` window for the named world, and the deletion happens only when the player presses that window's OK button; Cancel, or closing the window any other way, deletes nothing (`UltiKits/UltiWorlds#19`). The console cannot run this command at all — `WorldCommand` is `@CmdTarget(PLAYER)`, so the framework refuses a console sender with its player-only message before the method runs. Never deletes through a symbolic link, wherever the link is found: a link among the world folder's children has only its entry removed, and so does a world folder that is itself a link — otherwise `File#listFiles()` resolves the link and the target's contents are deleted through it while only the link is reported as the world (`UltiKits/UltiWorlds#20`). A link is therefore not a route to a protected world's data under an unprotected name, and the console states when the named thing is a link, phrased as what the command can reach rather than as what it has already done, so the line remains true when the link cannot be removed; a failed unlink is then reported as a link still in the world container rather than as files remaining in a folder | command | `/world delete <name>` | ultiworlds.admin.delete | player | admin | detailed | WorldCommand#deleteWorld, WorldDeleteConfirmPage#onConfirm, WorldService#deleteWorld |
