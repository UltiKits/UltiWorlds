...
| ultiworlds.world.load | Bring an on-disk-but-unloaded world back online, rebuilt with the environment it last had rather than as an overworld: the service uses the environment it recorded when it last created, loaded or unloaded that world, and failing that reads the dimension sub-folder the server wrote inside the world folder (`DIM-1` → NETHER, `DIM1` → THE_END, neither → NORMAL), so the environment also survives a restart (`UltiKits/UltiWorlds#22`, fixed). Accepts a name that is either currently loaded (no-op success) or present as an on-disk folder, unlike `## GUI`'s `ultiworlds.gui.world-list-page` which only ever lists already-loaded worlds | command | `/world load <name>` | ultiworlds.admin.load | player | admin | brief | WorldCommand#loadWorld, WorldService#loadWorld |
...
| ultiworlds.config.worlds.default_world | The world name treated as the server's default — `/world unload`/`/world delete` both refuse to act on it, and it is the fallback teleport target when kicking players out of a blocked/unloading world. `/world unload` and the `world.delete.default` message compare the name exactly; the deletion refusal itself is re-applied in `WorldService#deleteWorld` without regard to letter case, so it also covers a caller that is not the command | config | `config/worlds.yml: default_world (default: "world")` | n/a | n/a | admin | detailed | WorldCommand#unloadWorld, WorldCommand#deleteWorld, WorldCommand#blockWorld, WorldService#deleteWorld |
