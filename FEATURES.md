...
- **Kind**, exactly these eight values: `command`, `config`, `event`, `gui`, `scheduled`,
  ...
  The two rows under `## Lifecycle Hooks` are `event` rows with no `@EventHandler` site behind them:
  `/ul reload` and module enable are framework-invoked lifecycle steps, not commands this repository
  maps or config reads, so `event` is the closest-fitting Kind.
...
This module declares no unload hook of its own, and one reload hook, `onReload()`, whose only work is
the removed-key check of `ultiworlds.lifecycle.removed-key-warning`. Before `UltiKits/UltiWorlds#27` it overrode
...
reload row's Source cell names that inherited framework member.
