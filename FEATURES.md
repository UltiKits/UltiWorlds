...
- **Target**, exactly four: `player`, `console`, `both`, or `n/a` — the first three read straight
  ...
  `@CmdTarget(PLAYER)` on 20 of its 21 mappings, so those 20 rows record Target `player`; the one
  mapping without a narrowing, `delete <name>`, records `both` (`UltiKits/UltiWorlds#19`).
